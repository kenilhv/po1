...
async def run_invoice_pipeline(
    ...
    if not parsed.get("vendor_name") and not parsed.get("invoice_number"):
        from pathlib import Path

        stem = Path(pdf_path).stem.replace("_", " ").replace("-", " ").title()
        parsed.setdefault("vendor_name", stem or "Unknown Vendor")
        parsed.setdefault("invoice_number", Path(pdf_path).stem.upper())
    await emit(
        on_agent,
        agent="invoice_parser",
        status="complete",
    ...
    with get_conn() as conn:
        conn.execute(
            """
            UPDATE invoices SET vendor_name=?, amount=?, invoice_number=?,
