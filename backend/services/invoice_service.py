...
def list_invoices(*, risk: str | None = None, limit: int = LIST_LIMIT) -> list[dict[str, Any]]:
    query = "SELECT * FROM invoices ORDER BY created_at DESC LIMIT ?"
    params: tuple = (limit,)
    if risk:
        query = "SELECT * FROM invoices WHERE risk_score = ? ORDER BY created_at DESC LIMIT ?"
        params = (risk.upper(), limit)
    with get_conn() as conn:
...
def decisions_grouped() -> dict[str, list[dict[str, Any]]]:
    ...
    with get_conn() as conn:
        rows = conn.execute(
            """
            SELECT id, invoice_number, filename, vendor_name, amount, status, decision, risk_score, created_at
            FROM invoices
    ...
    for r in rows:
        ...
        if risk not in out or len(out[risk]) >= LIST_LIMIT:
            continue
...
def pending_cfo(*, limit: int = LIST_LIMIT) -> list[dict[str, Any]]:
    with get_conn() as conn:
        rows = conn.execute(
            """
